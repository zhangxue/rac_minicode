...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current hr'      do logout_killing_session!; current_hr.should be_nil end
    it 'kills :hr_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:hr_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_hr.remember_me
      current_hr.remember_token.should_not be_nil; current_hr.remember_token_expires_at.should_not be_nil
      Hr.find(1).remember_token.should_not be_nil; Hr.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      Hr.find(1).remember_token.should     be_nil; Hr.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current hr'      do logout_keeping_session!; current_hr.should be_nil end
    it 'kills :hr_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:hr_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @hr[:remember_token]            = token; 
      @hr[:remember_token_expires_at] = time
      @hr.save!
    end    
    before do 
      @hr = Hr.find(:first); 
